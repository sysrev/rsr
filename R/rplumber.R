...
#' @param action either httr::GET httr::POST or any other httr action
#' @param token a sysrev token with read access to the given project
#' @return A dataframe
#'
rplumber = function(path,params=list(),action=httr::GET,token=keyring::key_get("sysrev.token")){
  req   <- action("http://localhost:8000/", path=path,query=params)
